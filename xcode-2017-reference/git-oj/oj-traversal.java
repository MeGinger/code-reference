...
Time complexity: when using Adjacency List to represent the Graph,
DFS time complexity is O(n+e). 
e is the number of Arc or Edge. 
...
class Solution {
    ...
    public boolean canVisitAllRooms(List<List<Integer>> rooms) {
        ...
        enterRooms(rooms, open, 0);
        System.out.println(open);
        
        return open.size() == rooms.size();
    }
    
    private void enterRooms(List<List<Integer>> rooms, Set<Integer> open, int room) {
        
        for (Integer next : rooms.get(room)) {
            if (!open.add(next)) { // filter visited nodes, works as visited 
                continue;
            }
            
            enterRooms(rooms, open, next);
        }
    }
}
